# Probe test ROC-AUC by layer (GPQA cues)
...
| Model | BBH | GPQA | MMLU |
|---|---|---|---|
| Gemma-3 4B | 0.84 | 0.68 | 0.87 |
| Qwen-3.5 9B | 0.65 | 0.65 | 0.90 |
| Gemma-3 12B | 0.69 | 0.64 | 0.78 |

**Probes are dataset-dependent:** strong on MMLU (0.71–0.98), moderate on BBH, near chance on GPQA (0.57–0.70) and the unified pools (0.57–0.60). `fig8` shows GPQA only (worst case); `fig8b` shows all datasets. Selected AUC = argmax over layers on the test set (optimistic); train AUC = 1.00 everywhere (overfit).
